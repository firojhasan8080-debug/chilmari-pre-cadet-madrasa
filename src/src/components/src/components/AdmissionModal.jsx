...
export default function AdmissionModal({ isOpen, onClose }) {
  ...
  const [activeCamera, setActiveCamera] = useState(null); // 'student', 'parent', 'nidFront', 'nidBack'
  ...
  const handleSendOtp = () => {
    ...
    setOtpSent(true);
    setOtpError('');
    alert(`[SMS সিমুলেশন]: চিলমারী প্রি ক্যাডেট মাদ্রাসা ভর্তির ওটিপি কোড: ${randomOtp}`);
  };
  ...
  const startCamera = async (type) => {
    ...
    try {
      ...
      alert('ক্যামেরা চালু করা সম্ভব হয়নি। অনুগ্রহ করে ব্রাউজারের ক্যামেরা পারমিশন চেক করুন।');
    }
  };

  // ক্যামেরা থেকে ছবি তোলার লজিক (AI / Quality Detection Check)
  const capturePhoto = (type) => {
    ...
    if (videoRef.current) {
      ...
      // NID এর জন্য ফটো স্ক্যান ভ্যালিডেশন লজিক
      if (type === 'nidFront' || type === 'nidBack') {
        // লজিক: ক্যামেরা ফ্রেমে ডকুমেন্ট আছে কিনা সিমুলেটেড চেক
  ...
  const handleSubmit = (e) => {
    ...
    alert('🎉 অভিনন্দন! আপনার ভর্তি আবেদন সফলভাবে জমা হয়েছে। অতি শীঘ্রই মাদ্রাসা কর্তৃপক্ষ আপনার সাথে যোগাযোগ করবে।');
    onClose();
  };
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', backgroundColor: '#f8fafc', padding: '12px 18px', borderRadius: '16px' }}>
          {[1, 2, 3, 4].map((num) => (
            <div key={num} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '12px', fontWeight: '600', color: step >= num ? '#14532d' : '#94a3b8', display: window.innerWidth < 500 ? 'none' : 'inline' }}>
                {num === 1 ? 'শিক্ষার্থী' : num === 2 ? 'অভিভাবক ও OTP' : num === 3 ? 'ঠিকানা' : 'ডকুমেন্ট ও ছবি'}
              </span>
            </div>
          ))}
        </div>

        {/* ফরম জমা দিন বা ধাপ পরিবর্তন */}
        <form onSubmit={handleSubmit}>
          
          {/* 🟢 ধাপ ১: শিক্ষার্থীর মৌলিক তথ্য */}
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <button type="button" onClick={() => { if(formData.studentName && formData.age && formData.class) setStep(2); else alert('অনুগ্রহ করে সকল স্টার চিহ্নিত ঘর পূরণ করুন'); }} style={{ width: '100%', padding: '12px', backgroundColor: '#16a34a', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer', marginTop: '10px' }}>
                পরবর্তী ধাপ ➔ (অভিভাবক তথ্য)
              </button>
            </div>
          )}

          {/* 🟢 ধাপ ২: অভিভাবক ও OTP ভেরিফিকেশন */}
          {step === 2 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
              ...
              <div style={{ backgroundColor: '#f0fdf4', padding: '16px', borderRadius: '12px', border: '1px solid #bbf7d0' }}>
                ...
                {phoneVerified && (
                  <div style={{ color: '#16a34a', fontWeight: 'bold', fontSize: '13px', marginTop: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    ✅ নম্বরটি সফলভাবে ভেরিফাই করা হয়েছে!
                  </div>
                )}

                {/* OTP চার ডিজিটের বক্স */}
                {otpSent && !phoneVerified && (
                  <div style={{ marginTop: '14px', paddingTop: '12px', borderTop: '1px dashed #abd6bc' }}>
                    <p style={{ fontSize: '12px', color: '#15803d', margin: '0 0 8px 0', fontWeight: '600' }}>আপনার মোবাইলে প্রেরিত ৪ ডিজিটের কোডটি দিন:</p>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button type="button" onClick={() => { if (formData.fatherName && formData.motherName && phoneVerified) setStep(3); else alert('অভিভাবকের তথ্য এবং ফোন নম্বর ওটিপি ভেরিফিকেশন করা বাধ্যতামূলক!'); }} style={{ width: '70%', padding: '12px', backgroundColor: '#16a34a', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}>
                  পরবর্তী ধাপ ➔ (ঠিকানা)
                </button>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div>
                  ...
                  <input type="text" required placeholder="গ্রাম বা এলাকার নাম" className="form-input" value={formData.village} onChange={e => setFormData({ ...formData, village: e.target.value })} />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                ...
                <button type="button" onClick={() => { if (formData.village) setStep(4); else alert('অনুগ্রহ করে সঠিক ঠিকানা লিখুন'); }} style={{ width: '70%', padding: '12px', backgroundColor: '#16a34a', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}>
          ...
          {/* 🟢 ধাপ ৪: লাইভ ক্যামেরা ছবি ও NID আপলোড (বাধ্যতামূলক) */}
          {step === 4 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              
              {/* লাইভ ক্যামেরা উইন্ডো */}
              {activeCamera && (
              ...
              {/* ৩. অভিভাবকের NID কার্ড (Front & Back) */}
              <div className="cam-box">
                <p style={{ margin: '0 0 6px 0', fontSize: '13px', fontWeight: 'bold' }}>৩. অভিভাবকের NID কার্ডের ছবি (AI ডকুমেন্ট স্ক্যান) <span>*</span></p>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                ...
                <button type="submit" style={{ width: '70%', padding: '12px', backgroundColor: '#16a34a', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer', boxShadow: '0 4px 14px rgba(22, 163, 74, 0.35)' }}>
